...
const eliminarArchivo = (idx) => {
  try { if (archivos.value[idx]?.previewUrl) URL.revokeObjectURL(archivos.value[idx].previewUrl); } catch(e) {console.log(e)}
...
const enviarOC = async () => {
  ...
  try {
    ...
    destinoCompra.value = "";
    for (const a of archivos.value) { try { if (a?.previewUrl) URL.revokeObjectURL(a.previewUrl); } catch(e){console.log(e)} }
    archivos.value = [];
    comentario.value = "";
