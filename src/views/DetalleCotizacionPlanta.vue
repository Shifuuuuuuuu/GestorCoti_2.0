<!-- src/views/DetalleCotizacionPlanta.vue -->
<template>
  <div class="container py-4 py-md-5">
    ...
    <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
      ...
      <h1 class="h4 fw-semibold mb-0">Detalle de Cotización Planta</h1>
    ...
    <div v-if="loading" class="text-center py-5">
      ...
      <div class="mt-2">Cargando cotización de planta…</div>
    </div>
    ...
    <template v-else>
      ...
      <div v-if="docData" class="d-grid gap-3">
        ...
        <div class="card card-elevated">
          <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="fw-semibold">
              Cotización Planta N° <span class="text-danger">{{ docData.id ?? "—" }}</span>
            </div>
            <div class="small">
            ...
            <div class="row g-3">
              ...
              <div class="col-12 col-md-6">
                ...
                <div class="fw-semibold">
                  ...
                  {{ docData.nombre_centro_costo || "—" }}
                </div>
              </div>

              <div class="col-12 col-md-6">
                <div class="small">Aprobador sugerido</div>
                <div class="fw-semibold">{{ docData.aprobadorSugerido || "Alejandro Candia" }}</div>
              </div>
              ...
              <div class="col-12 col-md-4">
                <div class="small">Tipo flujo</div>
                <div class="fw-semibold">{{ docData.tipo_flujo || "planta" }}</div>
              </div>
        ...
        <div v-if="canResubmitEditor" class="card border-2 border-danger-subtle">
          <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="fw-semibold text-danger">Subir nueva cotización de planta</div>
            <span class="small text-secondary">
              Puedes seleccionar múltiples PDF o imágenes. El estado volverá a “Pendiente de Aprobación”.
            </span>
          </div>
          ...
          <div class="card-body">
            ...
            <div class="row g-3">
              ...
              <div class="col-12 col-md-3">
                ...
                <input class="form-control" value="Pendiente de Aprobación" readonly />
                <small class="text-secondary">Siempre vuelve a aprobación.</small>
              </div>
              ...
              <div class="col-12">
                ...
                <textarea
                  ...
                  placeholder="Motivo de la nueva cotización…"
                ></textarea>
              </div>
            </div>
          </div>
        ...
        <div v-if="canUploadOCProveedorEditor" class="card border-2 border-primary-subtle">
          <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <div class="fw-semibold text-primary">Subir Archivos OC / Proveedor</div>
            <span class="small text-secondary">
              Puedes seleccionar múltiples PDF o imágenes. El estado pasará a “Enviada a proveedor”.
            </span>
          </div>
          ...
          <div class="card-body">
            ...
            <div class="row g-3">
              ...
              <div class="col-12 col-md-4">
                ...
                <input class="form-control" v-model="comentarioEnvio" placeholder="Ej: Enviado al proveedor" />
              </div>
            </div>
          </div>
        ...
        <!-- VINCULACIÓN SOLPED -->
        <div class="card" v-if="docData.tipo_solped || docData.solpedId || docData.numero_solpe">
          <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
            <span class="fw-semibold">Vinculación SOLPED Planta</span>
            <button
              v-if="docData.solpedId"
              class="btn btn-sm btn-outline-primary"
              @click="goSolped(docData.solpedId)"
            >
              Ver SOLPED
            </button>
          </div>
        ...
        <div class="card" v-if="tieneArchivos || editingDetalle">
          ...
          <div class="card-body">
            ...
            <div v-if="editingDetalle" class="mt-3">
              ...
              <div class="row g-3">
                <div class="col-12 col-md-6">
                  <label class="form-label">Reemplazar Archivos OC</label>
                  <input
                    id="fileEditArchivoOCPlanta"
                    class="form-control"
                    type="file"
                    multiple
                    accept="application/pdf,image/*"
                ...
                <div class="col-12 col-md-6">
                  <label class="form-label">Reemplazar Adjuntos</label>
                  <input
                    id="fileEditAdjuntosPlanta"
                    class="form-control"
                    type="file"
                    multiple
                    accept="application/pdf,image/*"
          ...
          <div class="modal-content shadow-lg border-0 modal-surface">
            ...
            <div class="modal-body px-4 pt-0 pb-3">
              ...
              <div class="mt-3 small text-muted">
                <i class="bi bi-info-circle me-1"></i>
                Si el archivo no pertenece a esta cotización, puede que no se pueda borrar en Storage,
...
const isEditor = computed(() => norm(roleEffective.value) === "editor" || norm(roleEffective.value) === "admin");

const responsableDoc = computed(() => {
  const d = docData.value || {};
  if (d.responsable) return d.responsable;
  if (d.usuario_solped) return d.usuario_solped;
...
async function reloadDoc() {
  ...
  const dref = doc(db, COT_COLLECTION, String(id));
...
onMounted(async () => {
  try {
    ...
    await reloadDoc();
    if (!docData.value) error.value = "No se encontró la cotización de planta.";
  } catch (e) {
    console.error(e);
...
const canResubmitBase = computed(() => {
  ...
  return s.includes("rechaz");
});

const canUploadOCProveedorBase = computed(() => {
  return norm(docData.value?.estatus) === "aprobado";
});
...
function startEditDetalle() {
  ...
  editAdjuntos.value = [];
  const a = document.getElementById("fileEditArchivoOCPlanta");
  if (a) a.value = "";
  const b = document.getElementById("fileEditAdjuntosPlanta");
  if (b) b.value = "";
}
...
function isOwnedByThisDoc(fileObj) {
  ...
  if (p && p.startsWith(`${COT_COLLECTION}/${dId}/`)) return true;

  const url = String(fileObj?.url || "");
  if (!url) return false;
  return url.includes(`${COT_COLLECTION}%2F${encodeURIComponent(dId)}%2F`);
}
...
async function uploadOne(file, folder, prefix) {
  ...
  const path = `${COT_COLLECTION}/${dId}/${folder}/${prefix}_${Date.now()}_${safeName(file.name)}`;
...
async function saveEditDetalle() {
  try {
    ...
    const dref = doc(db, COT_COLLECTION, dId);
...
async function performDelete(key, idx) {
  try {
    ...
    if (key === "archivoOC") {
      ...
      const dref = doc(db, COT_COLLECTION, docData.value.__docId);
...
const resetForm = () => {
  ...
  submitOk.value = false;
  submitError.value = "";
  const el = document.getElementById("fileNuevaCotiPlanta");
  if (el) el.value = "";
};
  ...
  try {
    ...
    const uploaded = await Promise.all(
      filesNuevaCoti.value.map(async (f) => {
        const path = `${COT_COLLECTION}/${docData.value.__docId}/cotizacion_${Date.now()}_${safeName(f.name)}`;
        const fileRef = sref(storage, path);
        const up = await uploadBytes(fileRef, f);
        const url = await getDownloadURL(up.ref);
        return {
          nombre: f.name,
...
const resetEnvio = () => {
  ...
  sendOk.value = false;
  sendError.value = "";
  const el = document.getElementById("fileOCProveedorPlanta");
  if (el) el.value = "";
};
  ...
  try {
    ...
    const uploaded = await Promise.all(
      filesOCProveedor.value.map(async (f) => {
        const path = `${COT_COLLECTION}/${docData.value.__docId}/oc/oc_enviada_${Date.now()}_${safeName(f.name)}`;
        const fileRef = sref(storage, path);
        const up = await uploadBytes(fileRef, f);
        const url = await getDownloadURL(up.ref);
        return {
...
function goSolped(solpedId) {
  if (!solpedId) return;

  try {
    router.push({ name: "DetalleSolpedPlanta", params: { id: solpedId } });
  } catch {
    try {
      router.push({ name: "solped-planta-detalle", params: { id: solpedId } });
    } catch {
      router.push(`/solped-planta/${solpedId}`);
    }
  }
}
...
const estadoBadgeClass = (estatus) => {
  ...
  if (s.includes("edit")) return "bg-info-subtle text-info-emphasis";
