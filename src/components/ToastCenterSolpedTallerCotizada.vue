<template>
  <div class="toast-container position-fixed end-0 p-3" style="z-index:1080; top: 34%;">
    <div
    ...
    >
      <div class="toast-header">
        <i class="me-2" :class="['bi', t.icon || 'bi-tools']"></i>
        <strong class="me-auto">SOLPED Taller</strong>
...
const safeToasts = computed(() =>
  toasts.items
    .filter(t => t.channel === 'solped_taller_cotizada')
    .map(t => {
      let _to = null
      if (t.to) {
        try {
          ...
          console.error('[ToastCenterSolpedTallerCotizada] Ruta inválida:', e)
        }
      }
      return { ...t, _to }
    })
