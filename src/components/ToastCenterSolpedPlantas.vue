<template>
  <div class="toast-container position-fixed end-0 p-3" style="z-index:1080; top: 60%;">
    <div
    ...
    >
      <div class="toast-header">
        <i class="me-2" :class="['bi', t.icon || 'bi-building-fill-add']"></i>
...
const safeToasts = computed(() =>
  toasts.items
    .filter(t => t.channel === 'solped_plantas')
    .map(t => {
      let _to = null
      if (t.to) {
        try {
          ...
          console.error('[ToastCenterSolpedPlantas] Ruta inválida:', e)
        }
      }
      return { ...t, _to }
    })
