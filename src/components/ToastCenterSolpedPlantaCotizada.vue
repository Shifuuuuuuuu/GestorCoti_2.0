<template>
  <div class="toast-container position-fixed end-0 p-3" style="z-index:1080; top: 50%;">
    <div
    ...
    >
      <div class="toast-header">
        <i class="me-2" :class="['bi', t.icon || 'bi-building-check']"></i>
        <strong class="me-auto">SOLPED Plantas</strong>
...
const safeToasts = computed(() =>
  toasts.items
    .filter(t => t.channel === 'solped_planta_cotizada')
    .map(t => {
      let _to = null
      if (t.to) {
        try {
          ...
          console.error('[ToastCenterSolpedPlantaCotizada] Ruta inválida:', e)
        }
      }
      return { ...t, _to }
    })
